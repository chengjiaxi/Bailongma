...
export function buildSystemPrompt({
  ...
  // 参数仍接收以保留向后兼容签名，但不影�?system 字符串字节�?
...
2. **Decode "免密 / 默认 / 老地�?/ 老规�?/ 上次那个 / 你猜" as explicit signals** that the answer already exists locally or in memory. These phrases mean "go look", not "ask me again".
...
- The accompanying send_message must be at most a few characters �?e.g. "播放�?�?开始了"�?打开�?�?�?。No subject, no object, no explanation, no follow-up question.
...
export function buildContextBlock({
  ...
  //   currentTime    �?当前 ISO 时间�?
  //   existenceDesc  �?"X 小时 Y 分钟" 之类的存活描�?
  //   systemEnv      �?根据消息触发的环境块（天�?系统/桌面/热点�?
  //   currentChannel �?本轮 incoming 消息�?normalized channel（TUI/WECHAT/DISCORD/...�?
  ...
  // 本轮入口渠道：用户从哪个 channel 发来这条消息，决定你�?感知"到什么�?
  // 这块紧贴 current user message（contextBlock 会被 prepend �?current 内容前）�?
  // �?现在"/"那现在呢"这类代词追问优先解析�?channel 语义，而不是电池电量�?
...
There is no active current_task. Default to quiet presence, but do not treat quiet as paralysis. During TICK, if recent conversation, reminders, runtime context, or memory clearly indicate a heartbeat test, follow-up, useful report, or timely proactive action, you may act and send_message to a visible target. If nothing actually calls for action, wait.
  ...
  // 多帧栈语义：
  //   - 栈顶�?�?<focus>（当前主线）
  //   - 栈下面的�?�?<focus-history>（未完成的背景专注，可能已被压缩回填出结论）
  //   - 栈顶自己累积�?conclusions（子主题压缩回填上来的）也附�?<focus> 段末�?
  //
  // 向后兼容：旧调用点只�?focusFrame 时，把它当作单元素栈处理�?
  ...
  if (effectiveStack.length > 0) {
    ...
    if (top && Array.isArray(top.topic) && top.topic.length > 0) {
      ...
      // 栈顶自己�?conclusions：子主题压缩回填上来的「沉淀�?
    ...
    // 栈下面的�?�?<focus-history>：未完成的背景专�?
    if (effectiveStack.length > 1) {
      const historyLines = []
      // 从栈底到栈顶下方（不含栈顶），让最早的专注出现在最�?
...
${extraContext}
  ...
  // 时间词触发的轮廓注入：放�?<memories> 之前，作�?被相对时间词唤起的回�?�?
  // 内容�?focus_conclusion（每�?pop 时压成的 1-2 句话），不是对话原文�?
  // 块为空时整段不出现——平淡的一�?/ 用户没说相对时间词，就跟没这个机制一样�?
  if (temporalRecall) {
